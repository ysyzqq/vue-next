...
// 渲染器配置 抽象出一个renderer层, 根据用户自己定义的符合规范的vnode, element
...
>(
  ...
  function patch(
    ...
    // patching & not same type, unmount old tree n1,n2类型不一样直接卸载n1
  ...
  function mountChildren(
    ...
    for (let i = start; i < children.length; i++) {
      const child = (children[i] = optimized
        ? cloneIfMounted(children[i] as HostVNode) // 优化处理, 这里的结构是稳定的, 不做标准化
  ...
  function patchElement(
    ...
    if (patchFlag > 0) {
      ...
      // patchFlag存在说明这个元素的render函数是由compiler生成的, 并且可以快速处理
      if (patchFlag & PatchFlags.FULL_PROPS) {
        ...
        // faster iteration.
        // Note dynamic keys like :[foo]="bar" will cause this optimization to 动态key绑定属性会导致这里的优化失效
      ...
      // unoptimized, full diff 不优化,并且不是block
      patchProps(
        el,
        n2,
        oldProps,
    ...
    if (dynamicChildren != null) { // 有动态children, block优化处理
      patchBlockChildren(
        n1.dynamicChildren!,
        dynamicChildren,
        el,
  ...
  // The fast path for blocks. blocks快捷路径, 不做diff直接patch
  function patchBlockChildren(
    ...
    for (let i = 0; i < newChildren.length; i++) {
      ...
      patch(
        ...
        parentSuspense,
        isSVG,
        true // blocks里的vnode做优化处理, 减少一些diff
      )
    }
  }
  ...
  function processFragment(
    ...
    if (n1 == null) {
      ...
      if (patchFlag & PatchFlags.STABLE_FRAGMENT && dynamicChildren != null) {
        ...
        // 稳定的模板片段, 或是v-for修饰的, 不需要children相关的diff计算
        patchBlockChildren(
          n1.dynamicChildren!,
          dynamicChildren,
          container,
        ...
        // keyed / unkeyed, or manual fragments. 有无key, 或是自定义的template
    ...
    if (n1 == null) {
      ...
      if (shouldUpdateComponent(n1, n2, parentComponent, optimized)) {
        if (
          ...
          // normal update
          instance.next = n2
          // instance.update is the reactive effect runner. 组件实例的跟新就是effect()生成的runner(其实就是effct)主动执行一次, 调用模板函数渲染
          instance.update()
        }
      } else {
  ...
  function mountComponent(
    ...
    const Comp = initialVNode.type as Component // 组件配置或者函数组件
    ...
    // 生成组件的props,attrs,slots
    const propsOptions = Comp.props
    resolveProps(instance, initialVNode.props, propsOptions)
    resolveSlots(instance, initialVNode.children) // 在children里解析slots

    // setup stateful logic
    // 启动{}配置的组件, 启动后data, renderContext都会被挂在(响应式对象), 在后续的renderEffect的renderComponentRoot里, 会触发依赖追踪
  ...
  function setupRenderEffect(
    ...
    // ysy这里就是组件跟新重新渲染的effect
    instance.update = effect(function componentEffect() {
      if (!instance.isMounted) {
        // ysy在这里调用组件的render函数, 添加依赖
        const subTree = (instance.subTree = renderComponentRoot(instance))
        // beforeMount hook
        if (instance.bm !== null) {
          invokeHooks(instance.bm)
        }
        patch(null, subTree, container, anchor, instance, parentSuspense, isSVG) // 递归patch子树
        ...
        if (next !== null) {
          updateComponentPreRender(instance, next) // 解析props, slots等
        }
  ...
  const render: RootRenderFunction<HostNode, HostElement> = (
    ...
    if (vnode == null) {
      ...
      patch(container._vnode || null, vnode, container) // 没有n1就是null, 是组件装载
    }
    flushPostFlushCbs()
    container._vnode = vnode // 设置根vnode缓存
  }
